...
$worksheet=$workbook->addWorksheet('Earnings and Deductions');
...
$worksheet->write(1, 3, 'Earnings and Deductions', $formatHeader);

// Create the header for the data starting @ row 4
$indexCol = 0;
$indexRow = 4;
$worksheet->write($indexRow, $indexCol++, 'ID', $formatReportHeader);
$worksheet->write($indexRow, $indexCol++, 'Pay Types', $formatReportHeader);
$worksheet->write($indexRow, $indexCol++, 'PID', $formatReportHeader);
$worksheet->write($indexRow, $indexCol++, 'Emp Names', $formatReportHeader);
...
$indexCol = 0; // Start @ column 0
    ...
    $pmonth = $_REQUEST[payMonth];
    $ptype = $_REQUEST[payType];
    $pstCode=$_REQUEST[pstCode];
//    $pstCode2=$_POST['pstCode2'];
   ...
   $sql = 'SELECT p.`ID`,p.`Pid`,p.`emp_names`,p.`pay_type`,p.`amount`,p.`payDate`,p.`payMonth` FROM `proll_payments` p 
        ...
        where p.paymonth like "'.$pmonth.'%"';
   
    if($ptype<>''){
       $sql = $sql . " and p.`catID` like '$ptype%'";
    }
...
$workbook->send('earnings and deductions '.date('Hms').'.xls');
