...
//    $page = new Zend_Pdf_Page(Zend_Pdf_Page::SIZE_A4);
   ...
   // $page->drawText($bankTitle, $leftPos + 150, $topPos - 30);
    ...
    $page->drawText('Bank Code:', $leftPos + 170, $topPos - 60);
    $page->drawText('Account No:', $leftPos + 230, $topPos - 60);
    $page->drawText('Bank Name ', $leftPos + 300, $topPos - 60);
    $page->drawText('Branch', $leftPos + 420, $topPos - 60);
...
//    $page->drawLine($leftPos + 12, $topPos - 105, $leftPos + 550, $topPos - 105, Zend_Pdf_Page::SHAPE_DRAW_STROKE);
    ...
    $sql = "SELECT DISTINCT p.`PID`, p.`FirstName`, p.`Surname`, p.`LastName`, p.`Account_No`, b.`BankName`, e.`BankBranch` ,p.`ID_No`,b.bankcode
   ...
   // $page->drawText($row1['BankName'].' '.$row1['BankBranch'], $leftPos + 200, $topPos - 50);
    ...
    while ($row = $result->FetchRow()) {
        ...
        $page->drawText($row['bankcode'], $leftPos + 170, $topPos - $currpoint);
        $page->drawText($row['Account_No'], $leftPos + 230, $topPos - $currpoint);
        $page->drawText($row['BankName'] , $leftPos + 300, $topPos - $currpoint);
        $page->drawText($row['BankBranch'] , $leftPos + 420, $topPos - $currpoint);
...
//            echo $sql1;
    ...
    $page->setLineWidth(0.5);
    $page->drawLine($leftPos + 460, $topPos-75, $leftPos + 540, $topPos-75, Zend_Pdf_Page::SHAPE_DRAW_STROKE);
    $page->drawLine($leftPos + 460, $topPos-87, $leftPos + 540, $topPos-87, Zend_Pdf_Page::SHAPE_DRAW_STROKE);
    $page->drawLine($leftPos + 460, $topPos-90, $leftPos + 540, $topPos-90, Zend_Pdf_Page::SHAPE_DRAW_STROKE);
